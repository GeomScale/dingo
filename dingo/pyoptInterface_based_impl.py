...
def fva(lb, ub, S, c, opt_percentage=100):
    ...
    # declare the tolerance that highs, gurobi and copt works properly (we found it experimentally)
    tol = 1e-06
    ...
    try:
        ...
        model.add_linear_constraint(
            dot(c,v), poi.Geq, adjusted_opt_threshold
            )
...
def set_model(n, Aeq, beq, A, b):
    """
    A helper function of remove_redundant_facets function
    Create a PuLP model with given PuLP variables, equality constraints, and inequality constraints
    ...
    for i in range(Aeq.shape[0]):
        model.add_linear_constraint(
            dot(Aeq[i], x), poi.Eq, beq[i]
        )
        
    # Add the inequality constraints
    for i in range(A.shape[0]):
        model.add_linear_constraint(
            dot(A[i], x), poi.Leq, b[i]
        )
...
def remove_redundant_facets(lb, ub, S, c, opt_percentage=100):
    ...
    try:
        ...
        while removed > 0 or offset > 0:
            ...
            model_iter, v = set_model(n, Aeq, beq, np.array([-c]), [val])
            
            for j in range(n):
                model_iter.set_variable_attribute(v[j], poi.VariableAttribute.PrimalStart,max_biomass_flux_vector[j])
            ...
            for i in indices:
                ...
                for j in range(n):
                    model_iter.set_variable_attribute(v[j], poi.VariableAttribute.LowerBound, lb[j])
                    model_iter.set_variable_attribute(v[j], poi.VariableAttribute.UpperBound, ub[j])
                ...
                # if optimized
                status = model_iter.get_model_attribute(poi.ModelAttribute.TerminationStatus)
                if status == poi.TerminationStatusCode.OPTIMAL:
                ...
                if not facet_right_removed[i]:
                    ...
                    model_iter.set_variable_attribute(v[i], poi.VariableAttribute.UpperBound, ub[i]+1)
                    ...
                    model_iter.set_variable_attribute(v[i], poi.VariableAttribute.UpperBound, ub[i])
                ...
                if not facet_left_removed[i]:
                    # Relax the inequality
                    model_iter.set_variable_attribute(v[i], poi.VariableAttribute.LowerBound, lb[i]-1)
