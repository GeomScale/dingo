...
class polytope_sampler:
    ...
    def get_polytope(self):
        ...
        if (self.A == [] or self.b == [] or self.N == [] or self.N_shift = []):
            ...
            if self.A.shape[0] != self.b.size or self.A.shape[1] != Aeq.shape[1] or Aeq.shape[0] != beq.size:
                raise Exception("FVA failed.")

            self.A = np.vstack((A, -self.metabolic_network.biomass_function))

            self.b = np.append(
                self.b, -(self.parameters['opt_percentage'] / 100) * self.parameters['tol'] * math.floor(max_biomass_objective / self.parameters['tol'])
            )
    ...
    @staticmethod
    def sample_from_fva_output(min_fluxes, max_fluxes, max_biomass_flux_vector, max_biomass_objective, S, opt_percentage = 100, tol = 1e-06, ess = 1000, psrf = False, parallel_mmcs = False, num_threads = 1):

        A, b, Aeq, beq = get_matrices_of_low_dim_polytope(S, min_fluxes, max_fluxes, opt_percentage, tol)

        A = np.vstack((A, -max_biomass_flux_vector))
        b = np.append(
